@testset "Constraints" begin
    ...
    function setstate!(state, μ, d, constraints, method)
        state.μ = μ
        Optim_gf.update_fgh!(d, constraints, state, method)
    end

    @testset "Bounds parsing" begin
        b = Optim_gf.ConstraintBounds([0.0, 0.5, 2.0], [1.0, 1.0, 2.0], [5.0, 3.8], [5.0, 4.0])
        @test b.eqx == [3]
        @test b.valx == [2.0]
        ...
        b = Optim_gf.ConstraintBounds(Float64[], Float64[], [5.0, 3.8], [5.0, 4.0])
        ...
        ba = Optim_gf.ConstraintBounds([], [], [5.0, 3.8], [5.0, 4.0])
        @test eltype(ba) == Float64

        @test_throws ArgumentError Optim_gf.ConstraintBounds(
            [0.0, 0.5, 2.0],
            [1.0, 1.0, 2.0],
            [5.0, 4.8],
            [5.0, 4.0],
        )
        @test_throws DimensionMismatch Optim_gf.ConstraintBounds(
            [0.0, 0.5, 2.0],
            [1.0, 1.0],
            [5.0, 4.8],
            [5.0, 4.0],
    ...
    @testset "IPNewton computations" begin
        ...
        function pack_vec(x, b::Optim_gf.BarrierStateVars)
            n = length(x)
        ...
        function check_autodiff(d, bounds, x, cfun::Function, bstate, μ)
            ...
            p = Optim_gf.pack_vec(x, bstate)
            ftot! =
                (storage, p) -> Optim_gf.lagrangian_fgvec!(
                    p,
                    storage,
                    gx,
                    bgrad,
            ...
            xd = similar(x, TD)
            bstated = Optim_gf.BarrierStateVars{TD}(bounds)
            pcmp = similar(p)
            ftot = p -> Optim_gf.lagrangian_vec(p, d, bounds, xd, cfun, bstated, μ)
            #ForwardDiff.gradient!(pcmp, ftot, p, Optim_gf.ForwardDiff.{chunksize}())
        ...
        method = Optim_gf.IPNewton(μ0 = μ)
        options = Optim_gf.Options(; Optim_gf.default_options(method)...)
        ...
        ## No constraints
        bounds = Optim_gf.ConstraintBounds(Float64[], Float64[], Float64[], Float64[])
        bstate = Optim_gf.BarrierStateVars(bounds, x)
        bgrad = similar(bstate)
        f_x = NLSolversBase.value(dg, x)
        g_x = NLSolversBase.gradient(dg, x)
        L, ev = Optim_gf.lagrangian_fg!(
            gx,
            bgrad,
            bounds,
            x,
        ...
        state = Optim_gf.initial_state(method, options, dg, constraints, x)
        @test Optim_gf.gf(bounds, state) ≈ gx
        @test Optim_gf.Hf(constraints, state) ≈ H
        stateconvert = convert(Optim_gf.IPNewtonState{Float64,Vector{Float64}}, state)
        @test Optim_gf.gf(bounds, stateconvert) ≈ gx
        @test Optim_gf.Hf(constraints, stateconvert) ≈ H
        ## Pure equality constraints on variables
        xbar = fill(0.2, length(x))
        bounds = Optim_gf.ConstraintBounds(xbar, xbar, [], [])
        bstate = Optim_gf.BarrierStateVars(bounds)
        rand!(bstate.λxE)
        bgrad = similar(bstate)
        f_x = NLSolversBase.value(d0, x)
        g_x = NLSolversBase.gradient(d0, x)
        L, ev = Optim_gf.lagrangian_fg!(gx, bgrad, bounds, x, f_x, g_x, c, J, bstate, μ)
        @test f_x == 0
        ...
        state = Optim_gf.initial_state(method, options, d0, constraints, x)
        copyto!(state.bstate.λxE, bstate.λxE)
        setstate!(state, μ, d0, constraints, method)
        @test Optim_gf.gf(bounds, state) ≈ [gx; xbar - x]
        n = length(x)
        eyen = Matrix{Float64}(I, n, n)
        @test Optim_gf.Hf(constraints, state) ≈ [eyen -eyen; -eyen zeros(n, n)]
        ...
        bounds = Optim_gf.ConstraintBounds(zeros(length(x)), fill(Inf, length(x)), [], [])
        y = rand(length(x))
        bstate = Optim_gf.BarrierStateVars(bounds, y)
        rand!(bstate.λx)
        bgrad = similar(bstate)
        ...
        state = Optim_gf.initial_state(method, options, d0, constraints, y)
        setstate!(state, μ, d0, constraints, method)
        @test Optim_gf.gf(bounds, state) ≈ -μ ./ y
        @test Optim_gf.Hf(constraints, state) ≈ μ * Diagonal(1 ./ y .^ 2)
        # Now again using the generic machinery
        bounds = Optim_gf.ConstraintBounds([], [], zeros(length(x)), fill(Inf, length(x)))
        constraints = TwiceDifferentiableConstraints(cvar!, cvarJ!, cvarh!, bounds)
        state = Optim_gf.initial_state(method, options, d0, constraints, y)
        ...
        bounds = Optim_gf.ConstraintBounds(lb, ub, [], [])
        bstate = Optim_gf.BarrierStateVars(bounds, x)
        ...
        @test Optim_gf.gf(bounds, state) ≈ gxs
        @test Optim_gf.Hf(constraints, state) ≈ Diagonal(hxs)
        # Now again using the generic machinery
        bounds = Optim_gf.ConstraintBounds([], [], lb, ub)
        constraints = TwiceDifferentiableConstraints(cvar!, cvarJ!, cvarh!, bounds)
        state = Optim_gf.initial_state(method, options, d0, constraints, x)
        ...
        cbar = rand(length(c))
        bounds = Optim_gf.ConstraintBounds([], [], cbar, cbar)
        bstate = Optim_gf.BarrierStateVars(bounds, x, c)
        rand!(bstate.λcE)
        bgrad = similar(bstate)
        f_x = NLSolversBase.value(d0, x)
        g_x = NLSolversBase.gradient(d0, x)
        ...
        ch!(heq, x, bstate.λcE)
        @test Optim_gf.gf(bounds, state) ≈ [gx; cbar - c]
        @test Optim_gf.Hf(constraints, state) ≈ [
            Matrix(cholesky(Positive, heq)) -J'
            -J zeros(size(J, 1), size(J, 1))
        ]
        ## Nonlinear inequality constraints
        bounds =
            Optim_gf.ConstraintBounds([], [], .-rand(length(c)) .- 1, rand(length(c)) .+ 2)
        bstate = Optim_gf.BarrierStateVars(bounds, x, c)
        ...
        @test Optim_gf.gf(bounds, state) ≈ gf
        @test Optim_gf.Hf(constraints, state) ≈ Matrix(cholesky(Positive, hxx, Val{true}))
    end
    ...
    @testset "IPNewton initialization" begin
        ...
        state = Optim_gf.initial_state(method, options, d, constraints, x)
        Optim_gf.update_fgh!(d, constraints, state, method)
        ...
        @test Optim_gf.isinterior(constraints, x)
        state = Optim_gf.initial_state(method, options, d, constraints, x)
        Optim_gf.update_fgh!(d, constraints, state, method)
        @test norm(f_g - state.g_L_x) ≈ 0.01 * norm(f_g)
        ...
        @test Optim_gf.isfeasible(constraints, x)
        state = Optim_gf.initial_state(method, options, d, constraints, x)
        Optim_gf.update_fgh!(d, constraints, state, method)
        J = zeros(2, 4)
    ...
    @testset "IPNewton step" begin
        function autoqp(d, constraints, state)
            ...
            # have also called Optim_gf.solve_step!
            p = Optim_gf.pack_vec(state.x, state.bstate)
            ...
            Tx = typeof(state.x)
            stated = convert(Optim_gf.IPNewtonState{TD,Tx,1}, state)
            # TODO: Uncomment
            #stated2 = convert(Optim_gf.IPNewtonState{TD2, Tx, 1}, state)

            ϕd = αs -> Optim_gf.lagrangian_linefunc(αs, d, constraints, stated)
            # TODO: Uncomment
            #ϕd2 = αs->Optim_gf.lagrangian_linefunc(αs, d, constraints, stated2)
        ...
        state = Optim_gf.initial_state(method, options, d, constraints, [x0])
        qp = Optim_gf.solve_step!(state, constraints, options)
        ...
        αmax = Optim_gf.estimate_maxstep(
            Inf,
            state.x[bounds.ineqx] .* bounds.σx,
            state.s[bounds.ineqx] .* bounds.σx,
        )
        ϕ = Optim_gf.linesearch_anon(d, constraints, state, method)
        val0 = ϕ(0.0)
    ...
    @testset "Slack" begin
        ...
        for σ in (1, -1)
            ...
            state = Optim_gf.initial_state(method, options, d, constraints, [μ / F * 10])
            for i = 1:10
                Optim_gf.update_state!(d, constraints, state, method, options)
                state.μ = μ
                Optim_gf.update_fgh!(d, constraints, state, method)
            end
            ...
            state =
                Optim_gf.initial_state(method, options, d, constraints, [(1 + eps(1.0)) * σ])
            for i = 1:10
                Optim_gf.update_state!(d, constraints, state, method, options)
                state.μ = μ
                Optim_gf.update_fgh!(d, constraints, state, method)
    ...
    @testset "Constrained optimization" begin
        ...
        for (name, prob) in mcvp
            ...
            @test Optim_gf.minimum(results) < minval + sqrt(eps(minval))
        ...
        # Test the tracing
        # TODO: Update this when show_linesearch becomes part of Optim_gf.Options
