@testset "Particle Swarm" begin
    ...
    res = Optim_gf.optimize(f_s, initial_x, ParticleSwarm(lower, upper, n_particles), options)
    @test norm(Optim_gf.minimizer(res) - [5.0]) < 0.1

    initial_x = [0.0, 0.0]
    lower = [-20.0, -20.0]
    upper = [20.0, 20.0]
    n_particles = 5
    options = Optim_gf.Options(iterations = 300)
    ...
    @test norm(Optim_gf.minimizer(res) - [1.0, 1.0]) < 0.1
